...
export default function ExercisesPage() {
    ...
    const fetchByTarget = useCallback(async (target: string) => {
        ...
        setExercises([]);
        try {
            const res = await fetch(`/api/exercises?target=${encodeURIComponent(target)}&limit=12`);
    ...
    const fetchBySearch = useCallback(async (name: string) => {
        ...
        setExercises([]);
        try {
            const res = await fetch(`/api/exercises?name=${encodeURIComponent(name.trim())}&limit=12`);
                ...
                <div className="fixed inset-0 bg-slate-900/50 z-[60] flex items-end backdrop-blur-sm" onClick={() => setDetail(null)}>
                    ...
                    >
                        ...
                        <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-slate-100 mb-5">
                            <img
                                src={`/api/exercise-gif?id=${detail.id}`}
                                alt={detail.name}
                                className="w-full h-full object-cover"
                            />
                        </div>
